// Shadi Badir 
// I have implemented all system calls, including the cps call.
// With this implementation, we are able to print the name, process ID, state, parent's PID, external PID, and CPU time for each process listed in the processes table.
...
cps165() {
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
	 ...
	 int pid = p->ns_pid;
	 int ppid = pid == 1 ? 0 : p->parent->ns_pid;
